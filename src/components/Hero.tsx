...
export default function Hero() {
  ...
  return (
    <section ref={heroRef} id="hero"
      ...
      <div className="blob" style={{ width: "40vw", height: "40vw", background: "var(--yellow)", top: "-80px", right: "-5vw", opacity: 0.06 }} />
      <div className="blob" style={{ width: "35vw", height: "35vw", background: "var(--blue)", bottom: "80px", left: "-5vw", opacity: 0.05 }} />
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto text-center animate-stagger">
        ...
        <h1 className="font-extrabold tracking-tight mb-5 sm:mb-6" style={{ fontSize: "clamp(2.4rem, 8vw, 7rem)", lineHeight: 1.05 }}>
          {t.hero.headline1}{" "}
          <span style={{ fontStyle: "italic", color: "var(--yellow)", fontWeight: 400 }}>{t.hero.headlineItalic}</span>{" "}
          {t.hero.headline2}
        ...
        <p className="mx-auto mb-8 sm:mb-10 leading-relaxed text-sm sm:text-base lg:text-lg" style={{ color: "var(--text-secondary)", maxWidth: "560px" }}>
          {t.hero.sub}
        </p>
        ...
        <div className="flex flex-col items-center gap-2">
          <span className="font-mono text-xs tracking-widest uppercase" style={{ color: "var(--text-secondary)" }}>{t.hero.scroll}</span>
          <div className="w-px h-10 sm:h-12" style={{ background: "linear-gradient(to bottom, var(--yellow), transparent)", animation: "scroll-line 2s ease-in-out infinite" }} />
        </div>
      </div>
      ...
      <div className="absolute left-6 top-1/2 -translate-y-1/2 hidden xl:block" style={{ animation: "float 6s ease-in-out infinite" }}>
        <div className="glow-card rounded-2xl p-4 w-48" style={{ opacity: 0.7 }}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-mono" style={{ color: "var(--text-secondary)" }}>{t.hero.activeProject.toUpperCase()}</span>
          </div>
          <div className="space-y-2">
            {[80, 60, 90].map((w, i) => (<div key={i} className="h-1.5 rounded-full" style={{ width: `${w}%`, background: i === 1 ? "rgba(255,193,7,0.4)" : "rgba(255,255,255,0.1)" }} />))}
          </div>
        </div>
      </div>
      <div className="absolute right-6 top-1/3 hidden xl:block" style={{ animation: "float 8s ease-in-out infinite reverse" }}>
        <div className="glow-card rounded-2xl p-4 w-44" style={{ opacity: 0.6 }}>
          ...
          <div className="text-xs font-mono" style={{ color: "var(--text-secondary)" }}>{t.hero.avgGrowth.toUpperCase()}</div>
      ...
      <style jsx>{`
        @keyframes float { 0%,100%{transform:translateY(-50%) translateY(0px)} 50%{transform:translateY(-50%) translateY(-12px)} }
        @keyframes scroll-line { 0%{opacity:0;transform:scaleY(0);transform-origin:top} 50%{opacity:1;transform:scaleY(1)} 100%{opacity:0;transform:translateY(20px)} }
      `}</style>
    </section>
  );
}